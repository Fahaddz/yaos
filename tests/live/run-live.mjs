...
async function main() {
	...
	try {
		...
		await runCommand("node", [
			"tests/live/schema-guard.mjs",
		], token);
		await runCommand("node", [
			"tests/live/provider-manual-connect.mjs",
		], token);
		await runCommand("node", [
			"--import",
			"jiti/register",
		...
		await runCommand("node", [
			"tests/live/hardening-worker.mjs",
		], token);
		await runCommand("node", [
			"tests/live/ws-ticket-reconnect.mjs",
		], token);
		await runCommand("node", [
			"tests/live/ws-admission-protocol.mjs",
		], token);
	} catch (err) {
		if (output.trim()) {
			console.error("\n[wrangler output]");

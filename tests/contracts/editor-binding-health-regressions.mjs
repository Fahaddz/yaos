...
s.section("Test 1: validateOpenBindings uses repair-only flow");
{
	...
	s.check(section?.includes("editorBindings.repair("), "validateOpenBindings calls repair");
	s.check(!section?.includes("editorBindings.heal("), "validateOpenBindings does not call heal");
}
...
s.section("Test 2: bind unhealthy path uses repair, not heal");
{
	...
	s.check(section?.includes("if (this.repair(view, deviceName, `bind-health:${reason}`))"), "bind unhealthy path calls repair");
	s.check(!section?.includes("if (this.heal(view, deviceName, `bind-health:${reason}`))"), "bind unhealthy path does not call heal");
}
...
s.section("Test 3: maybeHealBinding uses repair/rebind and traces repair-only");
{
	...
	s.check(section?.includes("const repaired = this.repair("), "maybeHealBinding calls repair");
	s.check(!section?.includes("const healed = this.heal("), "maybeHealBinding does not call heal");
	s.check(section?.includes('? "repair-only"'), "health-restored action reports repair-only");
}

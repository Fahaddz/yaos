...
// than assigned. Node 20+ already exposes it; this only covers the gap.
...
s.section("Test 7: server-trace and free-form strings get scanned");
{
	...
	const diagnostic = out.serverTrace[0]?.diagnostic ?? "";
	s.check(
		!diagnostic.includes("Inbox/note.md"),
		"path embedded in server-trace free-form string is redacted",
	);
	s.check(
		/p:[a-f0-9]{32}/.test(diagnostic),
...
s.section("Test 13: post-redaction leak check catches a missed path");
{
	...
	const broken = createPassthroughRedactor().redactDeep(makeFixture());

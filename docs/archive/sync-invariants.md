...
Status: **Historical draft / design inventory** (2026-05-09).

This file is still useful as a rule catalog, but it is **not** the canonical
current-status document for the repo.
...
Important: many `pending`, `suspected_broken`, and `blocked` labels below reflect
the 2026-05-09 verification pass, not the current validated repo state.
...
`docs/rfcs/frontmatter-integrity.md` ("bound-file recovery now uses
...
`partial` in this historical draft. On 2026-05-09 this was treated as a
suspected round-trip surface based on `#22` symptoms; current repo status has
representative pass evidence for the `#22` family, but this specific invariant
still lacks a direct targeted proof. Verified: pending.
...
`docs/architecture/warts-and-limits.md` retention rationale). Verified: pending.
...
Current status: `target` in this historical draft. Since this pass, server-state
handoff coverage has improved and storage-level proof exists, but the full live
provider/client proof remains open (see FU-9a/FU-9b in
`docs/engineering/followups.md`). Verified: pending.
...
overall-state rules in `../engineering/sync-vocabulary.md`, but the facts are always
...
`implemented` per `docs/architecture/warts-and-limits.md` (fail-open).

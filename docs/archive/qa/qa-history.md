...
- [`docs/architecture/startup-load-and-blob-gate.md`](../../architecture/startup-load-and-blob-gate.md)
...
## Layer 4 harness history (May 2026)

The Layer 4 witness work produced a real harness, an offline bundle analyzer, and a
small pile of phase-signoff docs that have since been flattened into the main
engineering surface.

Current living docs for this work are:

- `docs/qa/layer4-harness-status.md`
- `docs/qa/multi-device-witness-runbook.md`

### Phase 2 — witness scaffold and first scenarios

Phase 2 established the core witness shape:
- in-memory witness segment buffer
- QA export/read path
- analyzer consumption of exported witness data
- first live two-device scenarios: `s11a` and `s11b`

Important historical outcomes:
- `s11a` proved passive lag without stale resurrection
- `s11b` clarified the conflict policy semantics:
  - disk wins the main file
  - displaced CRDT content goes to a local artifact on the affected device
  - the artifact itself is not expected to sync to the other device

Historical failure fixtures from that phase remain useful context:
- `qa-runs/s11a-failing-fixture/`
- `qa-runs/s11b-failing-no-artifact/`
- `qa-runs/s11b-failing-artifact-not-on-a/`

These fixtures explain why the harness evolved from strict quorum assumptions to
more nuanced convergence evidence.

### Phase 3 — witness bundles, offline analyzer, manual multi-device discipline

Phase 3 added the more durable and portable shape:
- witness bundle export
- QA identity command
- `scenarioRunId` and `scenarioStepIndex`
- offline bundle analyzer
- positive-evidence convergence rule (`analyzeConvergenceEvidence`)

Important historical note:
- some early three-device Phase 3 acceptance artifacts were synthetic fixtures that
  validated the bundle/analyzer pipeline, not full real-device three-device runs

That history matters because later status docs intentionally distinguish:
- **proven**
- **weak / partial**
- **not yet proven**

### Real-device Android open-editor result

The later `s13 Linux+Android` run answered the concrete mobile fear:
- Android had the note open
- Linux edited remotely
- Android converged with `editorHash == crdtHash == diskHash`
- no stale echo / old-hash recovery / persistent mismatch was observed

This result belongs to the live harness status doc now, but it is part of the
historical Layer 4 arc.

### Harness planning docs retired

Early harness planning lived in top-level `QA/` markdown files. Those were useful to
get the harness built, but the canonical implementation shape is now in:
- `qa/`
- `docs/qa/multi-device-witness-runbook.md`
- `docs/qa/layer4-harness-status.md`

The planning prose was retired once the implementation and operating docs existed.

## Regression-suite false failure incident (May 2026)

For one local repo state, `npm run test:regressions` appeared to show a seven-suite
ack/recovery failure cluster. That turned out not to be a product regression.

Root cause:
- stale compiled `.js` artifacts under `src/`
- jiti resolved those files first
- Node loaded a second Yjs instance
- state-vector and ack tests then failed for tooling reasons rather than product semantics

Outcome:
- stale artifacts were removed
- `guard:no-src-js-artifacts` was added to `test:regressions`
- current regression baseline returned to 60 passed / 0 failed

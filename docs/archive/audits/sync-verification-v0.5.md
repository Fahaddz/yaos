...
Status: **Historical verification snapshot**, not a current release-status
document.

This report captures what was true or suspected during the Phase 0.5 review pass.
It should be read as a time-bounded audit artifact.
...
Important: several gaps recorded here have since moved:
- the `#22` bug family now has representative fail/pass evidence and canonical status docs
- server receipt / SV echo work has landed at Level 3 MVP
- offline handoff proof is no longer "nothing exists"; FU-9a is addressed and FU-9b remains open

The matrix below still helps explain *why* certain follow-ups exist, but it should
not be treated as the current blocker list without cross-checking newer docs.
...
- Historical note: later repo evidence moved the `#22` family from "active
  reproduced failure" to "representative fail/pass coverage exists." What
  remains open is a narrower invariant-specific proof for this exact
  round-trip path.
- Blocks Phase 1 (as of this 2026-05-09 pass): **yes**.
...
- Historical note: this specific "pending count" claim is still not implemented
  in the current repo, but the stronger blanket statement "no receipt signal
  exists" is no longer current. Level 3 server receipt / SV echo work landed
  later; pending count remains intentionally unknown.
- Action (historical): design ack channel before this invariant can be honored.
...
- Status: `verified_missing`.
- Files read: same as `INV-OFFLINE-01`. At the time of this pass, no
  ack-level tracking existed.
- Evidence (historical): no `serverAcked`, `durable`, ack-channel, or
  per-update receipt in plugin or server code paths reviewed.
- Action:
...
- Historical note: Level 3 server receipt / SV echo support was implemented
  after this review. The remaining live gap is product/status integration and
  careful semantics, not total absence of receipt machinery.
- Blocks Phase 1 (as of this 2026-05-09 pass): **yes**.
...
- Evidence:
  ...
  - At the time of this pass, a field report (`#24`) meant the structural
    design could not be treated as sufficient proof by itself.
...
- Historical note: since this pass, storage-level offline handoff proof landed
  (`tests/offline-handoff.ts`, FU-9a). The remaining gap is the live
  provider/client integration proof (FU-9b), not total absence of handoff
  validation.
- Action (historical): build a two-client harness test that forces the issue
  `#24` scenario. Until that exists, this invariant is blocked from
  verification.
- Blocks Phase 1 (as of this 2026-05-09 pass): **yes**.
